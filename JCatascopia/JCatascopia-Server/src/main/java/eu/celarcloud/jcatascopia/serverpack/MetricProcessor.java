...
public class MetricProcessor implements Runnable{
	...
	private void sendRegisterRequest(String ip, String port, String header,String msg) {
    	...
    	try {			
	        ...
	        if (response != null) {
	            resp = response[1]; 
	            this.server.writeToLog(Level.INFO, resp);
	        }
    	}
    	catch(Exception e) {	
    		e.printStackTrace();
    		this.server.writeToLog(Level.INFO, "MetricProcessor>> " + e.getMessage());
    	}
    	finally {
    		dealer.close();
    	}
...
//					System.out.println("existing host: " + agentIP);
	...
	//					System.out.println("metric new in map: "+metric_name);
		...
		System.out.println("[test] " + server.agentMap.size() + " " + (System.currentTimeMillis()-this.tstart));
		return;
	}
}
